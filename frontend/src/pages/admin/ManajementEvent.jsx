...
const ManajemenEvent = () => {
  ...
  // 1. UPDATE STATE: Tambahkan field 'totalSeats'
  ...
  const handleSave = (e) => {
    ...
    // 2. SIMPAN DATA: Pastikan totalSeats tersimpan sebagai angka
    const dataToSave = {
        ...formData,
        price: parseInt(formData.price) || 0,
        totalSeats: parseInt(formData.totalSeats) || 0, // Simpan angka
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead style={{ background: '#F9FAFB', borderBottom: '1px solid #E5E7EB' }}>
            <tr>
              ...
              <th style={styles.th}>KUOTA (Seat)</th> {/* Kolom Baru */}
          ...
          <tbody>
            {events.map((ev) => (
              <tr key={ev.id} style={{ borderBottom: '1px solid #E5E7EB' }}>
                ...
                {/* Tampilkan Kuota Total */}
                <td style={styles.td}>
                    <span style={{fontWeight: 'bold'}}>{ev.totalSeats || 0}</span> Kursi
                </td>
        ...
        <div style={styles.modalOverlay}>
            <div style={styles.modalContent}>
                ...
                <form onSubmit={handleSave}>
                    ...
                    {/* INPUT BARU: TOTAL SEATS */}
                    <div style={{display:'flex', gap:'15px'}}>
                        <div style={{flex:1}}>
                            <label style={styles.label}>Harga Tiket (Start From)</label>
                            <input type="number" name="price" value={formData.price} onChange={handleChange} required style={styles.input} placeholder="150000" />
                        </div>
                        <div style={{flex:1}}>
                            <label style={{...styles.label, color: '#D97706'}}>Total Kuota (Seat)</label>
                            <input 
                                type="number" 
                                name="totalSeats" 
                                ...
                                required 
                                style={{...styles.input, border: '1px solid #F59E0B', background: '#FFFBEB'}} 
                                placeholder="Cth: 500" 
                            />
                            <small style={{fontSize:'10px', color: '#666'}}>*Kapasitas maksimal gedung</small>
                        </div>
                    </div>
